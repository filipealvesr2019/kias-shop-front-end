const CATEGORIES_DATA_URL = 'https://kias-shop-backend.onrender.com/api/categories';
const PRODUCTS_DATA_URL = 'https://kias-shop-backend.onrender.com/api/all-products';
  ...
  <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    ...
    ${categories
      .map(({ name }) => {
        return `
      <url>
        <loc>${`http://localhost:5015/categories/${name}`}</loc>
      </url>
    `;
      })
      .join('')}
      ...
      .map(({ name, _id }) => {
        return `
      <url>
        <loc>${`http://localhost:5015/products/${formatName(name)}/${_id}`}</loc>
      </url>
    `;
      })
      .join('')}
